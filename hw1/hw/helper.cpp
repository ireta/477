...
parser::Vec3f computeColor(int times, parser::Scene scene, std::vector<parser::Vec3f> ray)
{
    ...
    for(i=0 ; i<scene.spheres.size() ; ++i){
        t = intersectSphere(scene.spheres[i].radius, scene.vertex_data[scene.spheres[i].center_vertex_id-1], ray);
        //std::cout << t << std::endl;
        if(t<minT && t>0){
    ...
    if(minI1 != -1){
        ...
        for(i=0 ; i<scene.point_lights.size() ; ++i){
            ...
            if(LN > 0){
                ...
                dlight = multS(scene.point_lights[i].intensity, LN);
    ...
    for(i=0 ; i<scene.triangles.size() ; ++i){
        t = intersectTri(scene.vertex_data[scene.triangles[i].indices.v0_id-1], scene.vertex_data[scene.triangles[i].indices.v1_id-1], scene.vertex_data[scene.triangles[i].indices.v2_id-1], ray);
        if(t<minT && t>=0){
            //printVector(c);
						p1 = scene.vertex_data[scene.triangles[i].indices.v0_id-1];
						p2 = scene.vertex_data[scene.triangles[i].indices.v1_id-1];
						p3 = scene.vertex_data[scene.triangles[i].indices.v2_id-1];
						minI2 = i;
            minT = t;
        }
    }
    if(minI2 != -1){
				/*if(scene.triangles[minI2].nonormal){
						scene.triangles[minI2].Normal = normalize(CProduct(add(p2, multS(p1, -1)), add(p3, multS(p1, -1))));
						scene.triangles[minI2].nonormal = false;
				}
				N = scene.triangles[minI2].Normal;*/
    ...
    for(i=0 ; i<scene.meshes.size() ; ++i){
        for(j=0 ; j<scene.meshes[i].faces.size() ; ++j){
            ...
            t = intersectTri(scene.vertex_data[tri.indices.v0_id-1], scene.vertex_data[tri.indices.v1_id-1], scene.vertex_data[tri.indices.v2_id-1], ray);
            if(t<minT && t>=0){
            		//printVector(c);
                minI3 = i;
                minT = t;
								minI4 = j;
								p1 = scene.vertex_data[tri.indices.v0_id-1];
								p2 = scene.vertex_data[tri.indices.v1_id-1];
								p3 = scene.vertex_data[tri.indices.v2_id-1];
            }
        }
    }
    if(minI3 != -1){
    		...
    		intersectPoint = add(ray[0], multS(ray[1], minT));
				...
				/*if(scene.meshes[minI3].nonormal[minI4]){
						scene.meshes[minI3].Normal[minI4] = normalize(CProduct(add(p2, multS(p1, -1)), add(p3, multS(p1, -1))));
						scene.meshes[minI3].nonormal[minI4] = false;
				}
				N = scene.meshes[minI3].Normal[minI4];*/
				N = normalize(CProduct(add(p2, multS(p1, -1)), add(p3, multS(p1, -1))));
				reflect = multS(N, DProduct(N, ray[1])*-2);
				reflect = normalize(add(ray[1], reflect));
				for(i=0 ; i<scene.point_lights.size() ; ++i){
...
float intersectSphere(float radius, parser::Vec3f vertex, std::vector<parser::Vec3f> ray)
{
    ...
    C = DProduct(add(ray[0],multS(c,-1)), add(ray[0],multS(c,-1))) - radius*radius;
